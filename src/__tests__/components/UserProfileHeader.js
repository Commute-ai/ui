...
describe('UserProfileHeader', () => {
  ...
  it('renders user information when data is provided', () => {
    ...
    const { getByText } = render(<UserProfileHeader user={user} isLoading={false} />);
  ...
  it('renders an error message when an error occurs', () => {
    const { getByText } = render(<UserProfileHeader error="Failed to load" isLoading={false} />);
    const errorMessage = getByText('Failed to load');
    expect(errorMessage).toBeTruthy();
  });

  it('renders nothing when no user is provided and not loading', () => {
    const { queryByTestId } = render(<UserProfileHeader user={null} isLoading={false} />);
